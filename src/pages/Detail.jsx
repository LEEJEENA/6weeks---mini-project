...
import { __getmelon, __editTodo, __deleteTodo } from "../redux/modules/melonSlice"
            ...
            <Header/>
           {
            melon.map((melon)=> (
                melon.id === Number(id)&&(
                    ...
                    <STBox>
                        <div key={melon.id}>
                            <div>노래 / 제목 : {melon.singer} / {melon.song}</div>
                            ...
                            {edit ? (
                            <>
                            <STTextarea type="text" value={target} 
                                onChange={({ target }) => setTarget(target.value)} />
                            {/* 버튼 일렬로 정렬 */}
                            <div>
                            <Button onClick={()=> onClickUdapte(melon.id)}>수정완료</Button>
                            <Button onClick={()=> cancleButton()}>수정취소</Button>
                            </div>
                            
                            </> ) : (
                            <>
                            <STContent> <br/> {melon.content}</STContent>
                            <Button onClick={()=>{toggleEdit()}} >수정 </Button>
                            <Button onClick={()=> {onTodoDelete(melon.id) 
                            navigate("/")}
                            }>삭제하기</Button>
                            </> )}
                            </div>
